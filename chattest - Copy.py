...
def main():
    ...
    if not st.session_state.user_name:
        ...
        if st.session_state.user_name:
            st.chat_message("assistant").markdown(f"Hello {st.session_state.user_name}, how can I assist you today?")
            st.session_state.conversation.append({"role": "assistant", "content": f"Hello {st.session_state.user_name}, how can I assist you today?"})
        else:
            return
    ...
    if user_input := st.chat_input("Type your message here..."):
        ...
        with st.chat_message("assistant"):
            st.markdown(
                f"<p style='background-color: rgba(255, 255, 255, 0.5); padding: 10px; border-radius: 10px; margin: -10px 0 0 0;'> {full_response} </p>",
                unsafe_allow_html=True
            )
