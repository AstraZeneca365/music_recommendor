...
from transformers import pipeline # type: ignore # type: ignore
...
# Initialize session state for conversation history, user name, and previous chats
...
def main():
    ...
    if not st.session_state.user_name:
        ...
        if user_name:
            ...
            # Optionally, add a "New Chat" button after the greeting
            if st.button("Start New Chat"):
                # Save current conversation to previous chats
                if st.session_state.conversation:
                    st.session_state.previous_chats.append(st.session_state.conversation.copy())
                # Clear the current conversation
                st.session_state.conversation = []
                # Reset user name if desired, or keep it
                # st.session_state.user_name = ''
    else:
        ...
        if st.button("Start New Chat"):
            ...
            # Clear the current conversation
            st.session_state.conversation = []
            # Optionally, greet the user again or prompt for a new name
            st.session_state.conversation.append({"role": "assistant", "content": f"Hello again, {st.session_state.user_name}! How can I assist you today?"})
            display_conversation()
